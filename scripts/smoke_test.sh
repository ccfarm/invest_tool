...
if [ ! -d frontend/.next ]; then
  echo "缺少 frontend/.next，请先运行 npm run build" >&2
  exit 1
fi

cd frontend
PORT=80 PYTHON_BIN="$(pwd)/../backend/.venv/bin/python" npm start >/tmp/smoke_test.log 2>&1 &
SERVER_PID=$!
...
echo "==> /api/health"
curl -sf http://127.0.0.1:80/api/health | grep -q '"status":"ok"'
...
echo "==> GET /results?q=600519 SSR 页面"
